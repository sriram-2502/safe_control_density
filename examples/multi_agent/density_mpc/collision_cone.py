...
def main(argv=None):
    ...
    if args.scenario == "crossing2":
        args.horizon = max(args.horizon, 4)
        args.terminal_weight = max(args.terminal_weight, 1000.0)
        args.cone_density_margin = max(args.cone_density_margin, 0.55)
        args.mpc_safety_margin = max(args.mpc_safety_margin, 0.12)
        args.polish_activation_margin = max(args.polish_activation_margin, 0.20)
