...
def main():
    ...
    parser.add_argument("--steps", type=int, default=14000, help="Maximum simulation steps.")
    ...
    animation_path = Path("animations") / f"double_integrator_multi_qp.{animation_format}"
    ...
    for step in range(steps):
        ...
        candidate_indices = [
            idx
            for idx, obs in enumerate(control_obstacles)
            if _p_norm_distance(state[:2], obs) <= obs.r2 + 0.5
        ]
        candidate_indices.sort(
            key=lambda idx: _p_norm_distance(state[:2], control_obstacles[idx])
            - control_obstacles[idx].r1
        )
        active_indices = candidate_indices[:2]
        active_control_obstacles = [control_obstacles[idx] for idx in active_indices]
        active_accel_obstacles = [brake_obstacles[idx] for idx in active_indices]
        with timer:
            result = solve_double_integrator_density_qp(
                state,
                nav_goal,
                alpha,
                active_control_obstacles,
                v_des_prev,
                accel_obstacles=active_accel_obstacles,
                dt=dt,
        ...
        if step % print_interval == 0:
            print(
                ...
                f"speed={speed:.3f} "
                f"clearance={clearance:.3f} active={len(active_indices)} "
                f"slack={slacks[-1]:.2e}"
            )
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Double Integrator - Multiple Obstacles (Density QP)")
    ax.grid(True, linestyle="--", alpha=0.4)
