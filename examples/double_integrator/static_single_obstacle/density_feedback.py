...
def main():
    ...
    animation_path = Path("animations") / f"double_integrator_static.{animation_format}"
    ...
    def backstepping_control(pos, vel, prev_v_des):
        grad = density_grad(pos, goal, alpha, [inflated_obstacle])
    ...
    for step in range(steps):
        ...
        with timer:
            if dist < rad_from_goal:
                ...
                u, v_des_prev = backstepping_control(state[:2], state[2:], v_des_prev)
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Double Integrator - Static Obstacle (Density Feedback)")
    ax.grid(True, linestyle="--", alpha=0.4)
