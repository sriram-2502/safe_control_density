...
def _pose_density(state, goal_state, alpha, obstacles, reference_state=None, dt=None, theta_weight=0.05):
    err = _pose_error(state, goal_state)
    lyap = max(float(err[0] ** 2 + err[1] ** 2 + theta_weight * err[2] ** 2), 1e-6)
    is_next = reference_state is not None and float(np.linalg.norm(state[:2] - reference_state[:2])) > 1e-9
    phi = 1.0
    for obs in obstacles:
        center = obs.center + obs.velocity * float(dt) if is_next and dt is not None else obs.center
        phi *= p_norm_bump(state[:2], center, obs.r1, obs.r2, p=obs.p)
...
def _predicted_static_obstacles(moving_obstacles, predict_dt):
    return [
        Obstacle(
...
def _local_goal_planar(state, goal, radius, speed_floor, v_max):
    goal_vec = np.asarray(goal, dtype=float) - np.asarray(state[:2], dtype=float)
    dist = float(np.linalg.norm(goal_vec))
    if dist < 1e-10:
        return np.zeros(2, dtype=float)
    speed_scale = float(np.clip(dist / max(float(radius), 1e-9), float(speed_floor), 1.0))
    speed = float(v_max) * speed_scale
    return speed * goal_vec / dist
...
def main(argv=None):
    ...
    parser.add_argument("--goal-weight", type=float, default=0.6)
    parser.add_argument("--q-lqr", type=float, default=4.0)
    parser.add_argument("--r-lqr", type=float, default=1.0)
    parser.add_argument("--local-goal-radius", type=float, default=1.2)
    parser.add_argument("--local-speed-floor", type=float, default=0.12)
    parser.add_argument("--local-k-heading-scale", type=float, default=0.7)
    ...
    for step in range(args.steps):
        ...
        if step < scenario.start_delay_steps:
            ...
            static_obstacles = _predicted_static_obstacles(moving_obstacles, args.filter_dt)
            use_local_goal = args.scenario == "dense_flow" and dist < args.local_goal_radius
            if use_local_goal:
                planar_nominal = _local_goal_planar(
                    state,
                    goal,
                    args.local_goal_radius,
                    args.local_speed_floor,
                    args.v_max,
                )
                k_heading = args.local_k_heading_scale * args.k_heading
            else:
            ...
            nominal = _unicycle_from_planar(state, planar_nominal, args.v_max, args.omega_max, k_heading)
    ...
    if want_plot_data:
        plot_dynamic_obstacle_results(
            ...
            title="Dynamic Obstacles - Reactive Density Filter",
            xlim=scenario.xlim,
            ylim=scenario.ylim,
