...
def main(highlight_reactive_fov=False):
    ...
    for step in range(steps):
        ...
        if step % print_interval == 0:
            print(
                ...
                f"sensed={len(sensed)} slack={slacks[-1]:.2e} solve_ms={timer.last * 1e3:.1f}"
            )
    ...
    fov_active = np.asarray(fov_active, dtype=bool)
    ...
    summary = (
        ...
        f"max_sensed={max(sensed_counts, default=0)} max_buffered={max(buffered_counts, default=0)}"
    )
    if args.verbose:
